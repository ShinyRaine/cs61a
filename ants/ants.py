...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while p != beehive:
            ...
            if hasattr(self, 'min_range') and self.min_range >= n:
                p = p.entrance
                continue
            if hasattr(self, 'max_range') and self.max_range < n - 1:
                break
            if bee:
                return bee
            p = p.entrance
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if self.contained_ant is None:
            return not isinstance(other, ContainerAnt)
        return False
        # END Problem 9
...
# END Problem 13
        ...
        if self.true_queen is self:
            ...
            while p.exit != None:
                ...
                if p.ant != None:
                    ...
                    if isinstance(p.ant, ContainerAnt) and p.ant.contained_ant != None:
                        self.double_damage(p.ant.contained_ant)
        else:
            self.reduce_armor(self.armor)
    def double_damage(self, ant):
        if ant in self.doubled_damage:
            return
        ant.damage += ant.damage
        self.doubled_damage.append(ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.true_queen is self and self.armor == 0:
            bees_win()
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate, direction=1):
...
def apply_status(status, bee, length):
    ...
    def action(gamestate, *args):
        nonlocal length
        if length > 0:
            status_action(gamestate, *args)
            length -= 1
        else:
            origin_action(gamestate, *args)
    bee.action = action
    # END Problem EC
