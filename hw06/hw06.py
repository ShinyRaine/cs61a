...
def is_bst(t):
    ...
    if len(t.branches) == 2:
        ...
        r_min = bst_min(t.branches[1])
        return is_bst(t.branches[0]) and is_bst(t.branches[1]) and t.label >= l_max and t.label <= r_min
    if t.is_leaf():
        return True
    if t.branches[0].label > t.label:
        r_min = bst_min(t.branches[0])
        return is_bst(t.branches[0]) and t.label <= r_min
    if t.branches[0].label < t.label:
        r_max = bst_max(t.branches[0])
        return is_bst(t.branches[0]) and t.label >= r_max
